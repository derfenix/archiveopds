...
- **GitHub Actions:** [`.github/workflows/`](.github/workflows/) — on push/PR to `main` or `master`, `go test` and `go build`; on push of a `v*` tag (e.g. `v1.0.0`), [GoReleaser](https://goreleaser.com/) builds artifacts and publishes a GitHub Release (linux/windows/darwin, amd64/arm64, archives + `checksums.txt`).
